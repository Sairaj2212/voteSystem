...
import './AdminCss/MP.css';  // Ensure your CSS file is correctly linked
...
const MPCandidates = () => {
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch('http://localhost/vote_server/mp/delete_mp.php', {
        method: 'DELETE',
  ...
  const confirmAddCandidate = async () => {
    try {
      ...
      const response = await fetch('http://localhost/vote_server/mp/add_mp.php', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleUpdateCandidate = async () => {
    try {
      ...
      const response = await fetch('http://localhost/vote_server/mp/update_mp.php', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <>
      ...
      <div className={blur ? 'blurred' : ''}>
        <center>
        <h2 className='h2header'>MP Candidates Management</h2><h5>Total Members: {candidates.length}</h5><hr></hr>
      ...
      {showAddDialog && (
        <div className="dialog">
          <div className="closeWindow flex-container1">
            <h6 className='h6d'>{edit ? 'Edit MP Candidate' : 'Add MP Candidate'}</h6>
          ...
          <form onSubmit={handleSubmit} className="candidate-form">
            ...
            {!edit ? (
              ...
              <div className="edit-actions">
                <button className="save" onClick={()=> handleUpdateCandidate()} type="submit">Save</button>
